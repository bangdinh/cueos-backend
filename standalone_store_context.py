...
def require_permission(permission: str, jwt_payload: dict):
    ...
    # Chỉ OWNER mới bypass — SUPER_ADMIN phải có permission thực trong Composite Role
...
class StoreContext:
    ...
    def has_permission(self, permission: str) -> bool:
        # OWNER bypass toàn quyền; SUPER_ADMIN phải check permissions thực
...
def get_store_context(request: Request) -> StoreContext:
    ...
    if user_id_str and role:
        ...
        if role == UserRole.SUPER_ADMIN.value:
            ctx = StoreContext(user_id=user_id, role=role, store_id=None, permissions=perms or ALL_PERMISSIONS)
    ...
    # SUPER_ADMIN co 3 perms view_*, OWNER co 8 perms day du
