...
def main():
    ...
    print("Khoi dong AI Workers System (Real Webcam)...")
    for table_id, rtsp_url in tables:
        p = mp.Process(target=process_camera_stream, args=(table_id, rtsp_url))
        p.start()
        ...
        while True:
            for table_id, p in processes.items():
                if not p.is_alive():
                    ...
                    new_p = mp.Process(target=process_camera_stream, args=(table_id, rtsp_url))
                    new_p.start()
