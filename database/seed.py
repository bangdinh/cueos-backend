...
def seed_default_store_and_users(db: Session):
    ...
    if db.query(UserModel).filter(UserModel.username == "manager1").count() == 0:
        ...
        db.add(store_mgr)
        db.flush()
        db.add(UserStoreRole(user_id=store_mgr.id, store_id=1, role=UserRole.OWNER.value))
        db.commit()
    if db.query(UserModel).filter(UserModel.username == "staff1").count() == 0:
        store_staff = UserModel(username="staff1", password_hash="secret")
        db.add(store_staff)
        db.flush()
        db.add(UserStoreRole(user_id=store_staff.id, store_id=1, role=UserRole.MANAGER.value))
        db.commit()
    if db.query(UserModel).filter(UserModel.username == "manager2").count() == 0:
        ...
        db.add(store_mgr2)
        db.flush()
        db.add(UserStoreRole(user_id=store_mgr2.id, store_id=2, role=UserRole.OWNER.value))
        db.commit()
    if db.query(UserModel).filter(UserModel.username == "staff2").count() == 0:
        ...
        db.add(store_staff2)
        db.flush()
        db.add(UserStoreRole(user_id=store_staff2.id, store_id=2, role=UserRole.MANAGER.value))
        db.commit()
