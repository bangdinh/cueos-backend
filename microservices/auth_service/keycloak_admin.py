...
def get_user_by_username(username: str) -> Optional[str]:
    """Tìm UUID của user trong Keycloak theo username"""
    token = get_keycloak_admin_token()
    headers = {"Authorization": f"Bearer {token}"}
    url = f"{KEYCLOAK_URL}/admin/realms/{KEYCLOAK_REALM}/users"
    
    resp = requests.get(url, headers=headers, params={"username": username, "exact": "true"}, timeout=5)
    if resp.status_code != 200:
        raise RuntimeError(f"Lỗi tìm kiếm user '{username}' ({resp.status_code}): {resp.text}")
