...
class AssignRoleRequest(BaseModel):
    role_name: str = Field(..., description="Tên Custom Role (hoặc Role chuẩn) trên Keycloak")

@router.post("/api/stores/{store_id}/staff/{username}/roles")
def assign_custom_role_to_staff(
    store_id: int,
    username: str,
    ...
    """
    Gán một Client Role (Custom Role) trên Keycloak cho một nhân viên.
    Yêu cầu caller là OWNER của chi nhánh.
    """
    caller_username = user.get("preferred_username")
    client_roles = user.get("resource_access", {}).get("bida-app", {}).get("roles", [])
    realm_roles = user.get("realm_access", {}).get("roles", [])
    ...
    if "SUPER_ADMIN" in client_roles or "SUPER_ADMIN" in realm_roles:
        ...
        user_record = db.query(UserModel).filter(UserModel.username == caller_username).first()
        if user_record:
            store_role = db.query(UserStoreRole).filter(
                UserStoreRole.user_id == user_record.id,
                UserStoreRole.store_id == store_id,
    ...
    if not is_authorized:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"Chỉ OWNER của chi nhánh {store_id} mới có quyền gán Custom Role"
        )
        
    # Check if target user exists in SQLite and belongs to this store
    target_user = db.query(UserModel).filter(UserModel.username == username).first()
    if not target_user:
        raise HTTPException(status_code=404, detail="Không tìm thấy nhân viên trong hệ thống (SQLite)")
        
    target_store_role = db.query(UserStoreRole).filter(
        UserStoreRole.user_id == target_user.id,
        UserStoreRole.store_id == store_id
    ).first()
    if not target_store_role:
        raise HTTPException(status_code=403, detail="Nhân viên này không thuộc chi nhánh của bạn")

    # Call Keycloak API to assign role
    try:
        client_uuid = get_client_uuid("bida-app")
        
        # Verify the role exists
        role_rep = get_client_role(client_uuid, req.role_name)
        if not role_rep:
            raise HTTPException(status_code=404, detail=f"Không tìm thấy Role '{req.role_name}' trên Keycloak")
            
        # Security: can only assign roles prefixed with this store's ID or standard roles
        if req.role_name not in ["STAFF", "MANAGER", "OWNER"] and not req.role_name.startswith(f"STORE_{store_id}_"):
             raise HTTPException(status_code=400, detail="Chỉ được phép gán Role thuộc chi nhánh của bạn")
             
        kc_user_id = get_user_by_username(username)
        if not kc_user_id:
            raise HTTPException(status_code=404, detail=f"Không tìm thấy tài khoản Keycloak cho username '{username}'")
            
        add_client_roles_to_user(kc_user_id, client_uuid, [role_rep])
        
        return {"status": "success", "message": f"Đã gán role '{req.role_name}' cho '{username}' thành công"}
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Lỗi Keycloak Admin API: {str(e)}"
        )
