...
def login(req: LoginRequest, request: Request, db: Session = Depends(get_db)):
    ...
    user = db.query(UserModel).filter(UserModel.deleted_at == None).filter(UserModel.username == u_clean).first()
    if not user and u_clean in ["admin", "super_admin", "hq", "root"]:
        user = db.query(UserModel).filter(UserModel.deleted_at == None).filter(UserModel.username == "admin").first()
...
def revoke_tokens(target_user_id: int, request: Request, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Invalidate active sessions in-memory
